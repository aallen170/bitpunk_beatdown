...
public class Player2 : MonoBehaviour {
    ...
    void DetectDash()
    {
        if (moving && Input.GetKeyDown(gm.attack) && controller.collisions.below && !dashed && !slideAttacked)
        {
            dashed = true;
            if (facingLeft)
                dashedLeft = true;
    ...
    {
        if (inPushBack && (controller.collisions.left || controller.collisions.right || clinging) && canWallSplat && !stunned)
        {

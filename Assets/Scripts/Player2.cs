...
public class Player2 : MonoBehaviour {
	...
	void DetectDeath() {
		if (dead) {
			if (inLeftArea)
				print ("p2 died in left area");
			if (inRightArea)
				print ("p2 died in right area");
			if (inUpArea)
				print ("p2 died in up area");
			if (inDownArea)
				print ("p2 died in down area");
			deathSound.Play ();
			p1Score.gameScore++;
			if (p1Script.inLeftArea) {
				print ("p1 killed p2 in left area");
				transform.position = respawnRight.transform.position;
				dead = false;
			}
			if (p1Script.inRightArea) {
				print ("p1 killed p2 in right area");
				transform.position = respawnLeft.transform.position;
				dead = false;
			}
			if (p1Script.inUpArea) {
				print ("p1 killed p2 in up area");
				transform.position = respawnDown.transform.position;
				dead = false;
			}
			if (p1Script.inDownArea) {
				print ("p1 killed p2 in down area");
				transform.position = respawnUp.transform.position;
				dead = false;
			}
			if (dead) {
				print ("error: opponent not detected");
				dead = false;
			}
			if (inLeftArea)
				print ("p2 respawned in left area");
			if (inRightArea)
				print ("p2 respawned in right area");
			if (inUpArea)
				print ("p2 respawned in up area");
			if (inDownArea)
				print ("p2 respawned in down area");
		}
	}
